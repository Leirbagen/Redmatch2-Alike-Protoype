                ...
                if (TryGetComponent(out ChipOnFracture chipOnFractureComp))
                {
                    ...
                    if (unchippedRoot != null)
                    {
                        if (args.FracturePiecesRootObject != null)
                        {
                            ...
                            // The transform is already set on us, clear it on the children so that it is not double-applied
                            for (int i = 0, count = newRoot.transform.childCount; i < count; ++i)
                            {
                                Transform child = newRoot.transform.GetChild(i);

                                // There are both chipped and unchipped pieces. Each root already has our transform
