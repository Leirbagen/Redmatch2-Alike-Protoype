...
namespace DinoFracture
{
    ...
    public abstract class FractureGeometry : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        /// </summary>
        /// <remarks>
        /// Vertex merging only applies to vertices on the ring of a slice. The original hull
        /// of the mesh will never have vertices merged.
        /// </remarks>
        [Tooltip("Defines how aggressively to merge the vertices of this fracture. More aggressive merging will result in meshes that have a reduced number of vertices and triangles at the cost of more computation time during fracture.\n\nVertex merging only applies to vertices on the ring of a slice. The original hull of the mesh will never have vertices merged.")]
        public VertexMergingPolicy VertexMergingPolicy = VertexMergingPolicy.Default;
        ...
        private EntityId _lastMeshId = EntityId.None;
#else
        ...
        protected AsyncFractureResult StartFracture(Vector3 localPos, bool hideAfterFracture)
        {
            ...
            var details = CreateFractureDetails(localPos);
            if (details == null)
            {
                return null;
            }
            ...
            if (details is ShatterDetails shatterDetails)
            {
                ...
                {
                    Logger.Log(LogLevel.Debug, $"Invalid number of pieces or iterations. [Num Pieces: {shatterDetails.NumPieces}] [Num Iterations: {shatterDetails.NumIterations}]", gameObject);
                    return null;
                }
            }
        ...
        protected virtual FractureDetails CreateFractureDetails(Vector3 localPos)
        {
        ...
        private bool FillCommonDetails(FractureDetails details)
        {
            ...
            details.VertexMergingPolicy = VertexMergingPolicy;
        ...
        protected virtual AsyncFractureResult FractureInternal(Vector3 localPos)
        {
            return StartFracture(localPos, hideAfterFracture: true);
        }
        ...
        protected internal virtual void OnFracture(OnFractureEventArgs args)
        {
        ...
        /// <summary>
        /// Creates an empty set of OnFractureEventArgs that can be used
        /// to manually fracture completed handler in the event that calling
        /// Fracture() returns no async fracture result to attach to.
        /// </summary>
        public OnFractureEventArgs CreateEmptyFractureEventArgs()
        {
            return new OnFractureEventArgs(this, new Bounds(), null, null);
        }
        ...
        {
            return CreateSlicePlane(worldPlane.normal * -worldPlane.distance, worldPlane.normal, Vector3.up, targetGameObject);
        }

        internal static SlicePlaneSerializable CreateSlicePlane(Vector3 worldPos, Vector3 worldNormal, Vector3 localUp, Transform targetGameObject)
        {
            Vector3 localPos;
            Vector3 localNormal;
            ...
            {
                var localScale = targetGameObject.localScale;
                var invLocalScale = new Vector3(1.0f / localScale.x, 1.0f / localScale.y, 1.0f / localScale.z);
                var trans = targetGameObject.localToWorldMatrix * Matrix4x4.Scale(invLocalScale);
                trans = trans.inverse;

                var normalRot = Quaternion.LookRotation(worldNormal);
                Vector3 p0 = trans.MultiplyPoint(worldPos + normalRot * Vector3.right);
                Vector3 p1 = trans.MultiplyPoint(worldPos + normalRot * Vector3.up);
                Vector3 p2 = trans.MultiplyPoint(worldPos);
                localNormal = new Plane(p0, p1, p2).normal;

                localPos = targetGameObject.worldToLocalMatrix.MultiplyPoint(worldPos);
            }
            else
            {
                localPos = worldPos;
            ...
            ret.Position = localPos;
            ret.Rotation = Quaternion.LookRotation(localNormal, localUp);
            ret.Scale = 1.0f;
...
#if UNITY_6000_5_OR_NEWER
            if ((_meshValidity == MeshValidity.Unknown) || mesh == null || _lastMeshId != mesh.GetEntityId())
#else
            if ((_meshValidity == MeshValidity.Unknown) || mesh == null || _lastMeshId != mesh.GetInstanceID())
#endif
            {
#if UNITY_6000_5_OR_NEWER
                _lastMeshId = (mesh != null) ? mesh.GetEntityId() : EntityId.None;
#else
