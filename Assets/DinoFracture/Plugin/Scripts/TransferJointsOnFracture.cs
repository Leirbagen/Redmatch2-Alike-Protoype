...
namespace DinoFracture
{
    ...
    public class TransferJointsOnFracture : MonoBehaviour
    {
        ...
        /// scoped as possible.
        /// </summary>
        [UnityEngine.Tooltip("The tree to crawl in search for joints of other objects that need to be transferred to this joint. This search root should be as scoped as possible.")]
        ...
        /// </summary>
        [UnityEngine.Tooltip("How close this object must be to the joint in order to transfer. The larger the number, the more pieces will have joints transferred.")]
            ...
            if (origJoint != null)
            {
                ...
                for (int i = 0; i < rootTrans.childCount; i++)
                {
                    ...
                    Vector3 closestOnUs = (ourCollider != null)
                        ? ourCollider.ClosestPoint(worldPoint)
                        : transform.position;
                        ...
                        (closestOnUs - pieceTrans.position).sqrMagnitude + DistanceTolerance * DistanceTolerance)
                    {
                        ...
                        ourJoint.connectedAnchor = origJoint.connectedAnchor;
        ...
        {
            Rigidbody origRigidBody = args.OriginalObject.GetComponent<Rigidbody>();
            if (!origRigidBody)
            {
                return;
            }
            ...
            else
            {
                ...
                joints = FindObjectsByType<Joint>(FindObjectsInactive.Include);
#elif UNITY_2023_1_OR_NEWER
                joints = FindObjectsByType<Joint>(FindObjectsSortMode.None);
#else
            ...
            if (joints != null)
            {
                ...
                {
                    if (joints[i] != null && joints[i].connectedBody == origRigidBody)
                    {
                        Transform rootTrans = args.FracturePiecesRootObject.transform;
                        for (int j = 0; j < rootTrans.childCount; j++)
                        {
                            ...
                            if ((worldPoint - pieceTrans.position).sqrMagnitude <
                                (closestOnUs - pieceTrans.position).sqrMagnitude + DistanceTolerance * DistanceTolerance)
                            {
                                Vector3 worldConnectionPoint = origRigidBody.transform.localToWorldMatrix.MultiplyPoint(joints[i].connectedAnchor);

                                joints[i].connectedBody = pieceTrans.GetComponent<Rigidbody>();
                                joints[i].connectedAnchor = pieceTrans.worldToLocalMatrix.MultiplyPoint(worldConnectionPoint);
                            }
                        }
                    }
                }
