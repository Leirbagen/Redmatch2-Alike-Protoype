...
namespace DinoFracture
{
    ...
    public class ChipOnFracture : MonoBehaviour
    {
        ...
        /// it's important not to interfere with callbacks that might assume as certain
        /// tree structure.
        /// </summary>
        ...
        private void OnPostFracture(OnFractureEventArgs args)
        {
            ...
            {
                // If the root of the fracture already has a chipped fracture root
                // (as is the case when pre-fracturing at runtime), then there is
                // nothing left to do here.
                if (args.FracturePiecesRootObject.TryGetComponent(out ChippedFractureRoot _))
                {
                    return;
                }
                ...
                using (ListReference<FracturedObject>.Get(ref _sChippedObjectList))
                {
                    ...
                    {
                        GetChippedObjects(args, shatterDetails, isPreFracture: false);

                        var root = ProcessShatterChippedObjects(args, isPrefracture: false);
                        if (root != null)
                        {
        ...
        private void GetChippedObjects(OnFractureEventArgs args, ShatterDetails shatterDetails, bool isPreFracture)
        {
            float radius = 0.0f;
            if (!isPreFracture)
            {
                ...
                else if (shatterDetails.FractureSize.Value > 0)
                {
                    radius = shatterDetails.FractureSize.GetWorldSpaceSize(args.OriginalMeshBounds.size).magnitude * 0.5f;
                }
            }
        ...
        private ChippedFractureRoot ProcessShatterChippedObjects(OnFractureEventArgs args, bool isPrefracture)
        {
            ...
            if (_sUnchippedObjectList.Count > 0)
            {
                if (root == null)
                {
                    root = CreateNewRoot(null, 0, args, isPrefracture);
                }
                ...
                for (int i = 0; i < _sUnchippedObjectList.Count; i++)
                {
                    ...
                    AddChildToRoot(fractureChild, root, args, isPrefracture);
                }
            }
        ...
        private void ProcessChippedObjects(OnFractureEventArgs args, SliceDetails sliceDetails)
        {
            ...
            using (ListReference<ChippedFractureRoot>.Get(ref _sRootObjectList))
            {
                ...
                for (int i = 0; i < _sUnchippedObjectList.Count; i++)
                {
                    ...
                    if (root == null)
                    {
                        root = CreateNewRoot(curRoot, childId, args, isPrefracture: false);
                    }

                    AddChildToRoot(fractureChild, root, args, isPrefracture: false);
        ...
        internal GameObject OnPrefractureComplete(OnFractureEventArgs args)
        {
            ...
            using (ListReference<FracturedObject>.Get(ref _sChippedObjectList))
            {
                if (args.FractureDetails is ShatterDetails shatterDetails)
                {
                    GetChippedObjects(args, shatterDetails, isPreFracture: true);
                }
                else if (args.FractureDetails is SliceDetails sliceDetails)
                {
                    GetChippedObjects(args, sliceDetails);
                ...
                var root = ProcessShatterChippedObjects(args, isPrefracture: true);
        ...
        private void AddChildToRoot(FracturedObject unchippedChild, ChippedFractureRoot root, OnFractureEventArgs args, bool isPrefracture)
        {
            ...
            bool hasUserSpecifiedFractureGeom = (fractureGeom != null);

            if (fractureGeom == null && !isPrefracture && EnsureChildComponents)
            {
                fractureGeom = childGO.AddComponent<RuntimeFracturedGeometry>();
            }
            
            if (fractureGeom != null)
            {
                fractureGeom.ForceValidGeometry();

                if (!hasUserSpecifiedFractureGeom)
                {
        ...
        internal static void RecalculateRootProperties(ChippedFractureRoot root, GameObject fracturedChildToIgnore)
        {
            ...
            for (int i = 0; i < root.transform.childCount; i++)
            {
                ...
                if (child.gameObject.activeSelf && child.gameObject != fracturedChildToIgnore)
                {
                    ...
                    float childVolume = childFracturedObject.ThisVolume;
                    float childMass = childFracturedObject.ThisMass;
        ...
        private ChippedFractureRoot CreateNewRoot(ChippedFractureRoot curRoot, int subId, OnFractureEventArgs args, bool isPrefracture)
        {
            ...
            if (isPrefracture)
            {
                if (args.OriginalObject.FractureTemplate == null || !args.OriginalObject.FractureTemplate.TryGetComponent(out FractureGeometry _))
                {
                    // Pre-fractured pieces do not get runtime fracture geometry, even if EnsureChildComponents is true (unless explicitly added by the user).
                    // In the normal case, there won't be fracture callbacks, so pieces are manually broken off from the ChippedFractureRoot on collision.
                ...
                {
                    // Prefracture component has more fracture geometries underneath - let those handle chipping
                    chippedRoot.CollisionChipRadius = -1.0f;
                }
            }
            else
            {
                // Chipping radius is handled by fracture callbacks.
                chippedRoot.CollisionChipRadius = -1.0f;
            }
        ...
        private static string GetRootName(ChippedFractureRoot curRoot, int id, int subId)
        {
            ...
            return $"UnchippedRoot {curRoot.GetEntityId()} {id} {subId}";
#else
